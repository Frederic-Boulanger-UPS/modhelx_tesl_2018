...
class LaunchTESLSimulation implements ILaunchConfigurationDelegate {
    ...
    val static minorversion = 2
    val static releaseversion = 1
        ...
        try {
            ...
            for(Relation r : spec.getRelDecls()) {
                try {
            	    ...
            	    // spec.console(spec.err, e.message);
            	    errors = true
                }
            }
            ...
            if (errors) {
            	spec.console(spec.err,"## Execution aborted because of errors in the specification.");
            	spec.closeLog
            	spec.unload
            	return
            }
...
//                                          && (spec.stopClock.getNowTick() != null);
					...
					var stopClockHasTicked = spec.stopClock != null
					if (stopClockHasTicked) {
						val Tick<?> now = spec.stopClock.nowTick
						stopClockHasTicked = stopClockHasTicked && (now != null)
					}
                    ...
                    val tagv = if (spec.tagRefClock != null) {
                    	fr.supelec.tesl.core.Clock.getCurrentTagValue_erased(spec.tagRefClock)
                    } else {
                    	null
                    }
                    val tagd = try {
                        (tagv as Number).doubleValue()
                    } catch(Exception e) {
                        step - 1
                    }
                    spec.addTag(tagd);
