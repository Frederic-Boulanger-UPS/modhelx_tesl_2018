...
class ImplicationQualifierExtensions extends ExtensionWithExpressions {
    ...
    def dispatch void process(DelayedQualifier dq, Clock<? extends Comparable<?>> source, Clock<? extends Comparable<?>> target) {
        val delayClock = spec.getClock(dq.getClock().getName());
        val countValue = dq.getCount().evaluate();
        var count = countValue.intValue(dq, "delay value")
        source.impliesWithDelay(target, delayClock, count, dq.isImmediately(), dq.isReset())
    }
    def dispatch void process(SustainedQualifier sq, Clock<? extends Comparable<?>> source, Clock<? extends Comparable<?>> target) {
        ...
        source.sustains(target, startClock, stopClock, immediately, weakly)
    }
    def dispatch void process(FilteredQualifier fq, Clock<? extends Comparable<?>> source, Clock<? extends Comparable<?>> target) {
        ...
        source.impliesThroughFilter(target, skip, keep, repSkip, repKeep)
    }
    ...
    def dispatch void process(TimeDelayedQualifier tdq, Clock<? extends Comparable<?>> source, Clock<? extends Comparable<?>> target) {
		...
		var TimeDelayedImplication<? extends Comparable<?>,
                                   ? extends Comparable<?>,
                                   ?  extends Comparable<?>,
                                   ?  extends Comparable<?>> tdImplication = null 
        switch (clockType) {
            case TUNIT:
            ...
            case TINT: {
                ...
                tdImplication = source.impliesWithTimeDelay(resetClock, target, (measuring as Clock<BigInteger>), int_delay, true)
            }
            case TDEC: {
                val dec_delay = delay.bigDecimalValue(tdq, "measuring a delay on a decimal clock")
                tdImplication = source.impliesWithTimeDelay(resetClock, target, (measuring as Clock<BigDecimal>), dec_delay, true)
            }
            case TRAT: {
                val rat_delay = delay.rationalValue(tdq, "measuring a delay on a rational clock")
                tdImplication = source.impliesWithTimeDelay(resetClock, target, (measuring as Clock<Rational>), rat_delay, true)
            }
            case TFLOAT: {
                val double_delay = delay.doubleValue(tdq, "measuring a delay on a float clock")
                tdImplication = source.impliesWithTimeDelay(resetClock, target, (measuring as Clock<Double>), double_delay, true)
            }
            default:
    ...
    def dispatch void process(WhenQualifier wq, Clock<? extends Comparable<?>> source, Clock<? extends Comparable<?>> target) {
        ...
        if (wq.isNot()) {
            source.impliesWhenNot(when, target)
        } else {
            source.impliesWhen(when, target)
        }
    }
    def dispatch void process(NextQualifier nq, Clock<? extends Comparable<?>> source, Clock<? extends Comparable<?>> target) {
        ...
        source.sustains(target, nxt, source, ! nq.isStrict(), false)
    }
