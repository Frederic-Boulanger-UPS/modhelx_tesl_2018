...
class AwaitImplicationExtensions extends ExtensionWithExpressions {
    ...
    def process(AwaitImplication ai) {
        ...
        val Clock<?> reset = if (ai.reset != null) {
            spec.getClock(ai.reset.name)
        } else {
            null
        }
        val Clock<?> target = spec.getClock(ai.target.name)
        ...
        target.awaits_erased(reset, weak, masters.toArray(Array.newInstance(Clock, 0) as Clock<?>[]))
    }
}